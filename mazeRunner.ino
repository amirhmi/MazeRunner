...
void loop()
{
  ...
  Serial.print("\t");
  if ((headingDiff > THRESH || headingDiff < -THRESH) && state == forwardMove)
  {
    state = headingCorrection;
  }
  else if (state == forwardMove) {
    ...
    else
    {
      state = forwardMove;
      movedSteps = 0;
      startHeading = myHeading;
    }
  }
  else if (state == headingCorrection)
  {
    Serial.print("correction\t");
    if (headingDiff < SAFETHRESH && headingDiff > -SAFETHRESH)
    {
      state = forwardMove;
    }
